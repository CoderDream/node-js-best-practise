 ...
 *                  注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)。
 * 11	initgroups(user, extra_group) 读取 /etc/group ，并初始化群组访问列表，使用成员所在的所有群组。
 *                  这是授权操作，所有你需要有 root 权限，或者有 CAP_SETGID 能力。
 *                  注意：这个函数仅在 POSIX 平台上可用(例如，非Windows 和 Android)。
 * 12	kill(pid[, signal]) 发送信号给进程. pid 是进程id，并且 signal 是发送的信号的字符串描述。
 ...
 * 17	hrtime() 返回当前进程的高分辨时间，形式为 [seconds, nanoseconds]数组。它是相对于过去的任意事件。
